...
const SUPABASE_ANON = 'eyJhbGciOiJIUzI1NiIsInR5import { createClient } from '@supabase/supabase-js';
...
export async function sendNotification({ type, record, recipients }) {
  try {
    console.log('[notify] Sending:', type, 'to', recipients);
    const { data, error } = await supabase.functions.invoke('send-notification', {
      body: { type, record, recipients },
    });
    if (error) console.error('[notify] Edge Function error:', error);
    else console.log('[notify] Success:', data);
  } catch(e) { console.error('[notify] Failed:', e.message); }
}cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxzZ2F3eHpwaWxvdG90ZnN1bW1tIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzIxMDU3MTcsImV4cCI6MjA4NzY4MTcxN30.HLCaCqZ-YjywEhNC2e6LtR3haLivuZ13ukit1AeZ0QM';
